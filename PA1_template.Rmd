...
```{r dataload, message=F, warning=F, echo=T}
...
```{r loadpackages, message=F, warning=F, echo=T}
library(dplyr)
library(ggplot2)
library(gridExtra)
```
...
```{r meansteps, message=F, warning=F, echo=T}
...
```{r meanmissing, message=F, warning=F, echo=T}
newdata <- data
...
```{r meanvsmedian, message=F, warning=F, echo=T}
